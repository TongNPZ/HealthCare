...
import { InputField, TextAreaField, getCustomSelectStyles, SelectOption } from "../components/ui/FormFields";

import "react-phone-input-2/lib/style.css";
import "react-datepicker/dist/react-datepicker.css";

function PatientFormContent() {
  const { formMethods, isMounted, isSubmitted, onSubmit, t, router, nationalityOptions, languageOptions } = usePatientForm();
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8">
                  ...
                  <div className="flex flex-col gap-1.5 mb-5">
                    ...
                    <Controller name="gender" control={control} render={({ field }) => (
                      <Select {...field} options={[{ value: "Male", label: t("male") }, { value: "Female", label: t("female") }]} value={[{ value: "Male", label: t("male") }, { value: "Female", label: t("female") }].find(c => c.value === field.value)} onChange={(val) => field.onChange((val as SingleValue<SelectOption>)?.value ?? "")} styles={getCustomSelectStyles(!!errors.gender)} />
                    )} />
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5 mb-5">
                    ...
                    <Controller name="phoneNumber" control={control} render={({ field }) => (
                      <PhoneInput country={"th"} value={field.value} onChange={field.onChange} inputClass="!w-full !h-11 !text-slate-800 !border-slate-200 !bg-slate-50/50 !rounded-xl" />
                    )} />
                  </div>
                ...
                <div className="flex flex-col gap-1.5 mb-5">
                  ...
                  <Controller name="preferredLanguage" control={control} render={({ field }) => (
                    <Select {...field} isMulti options={languageOptions} value={languageOptions.filter(lang => field.value?.includes(lang.value))} onChange={(val) => field.onChange((val as MultiValue<SelectOption>).map(s => s.value))} styles={getCustomSelectStyles(!!errors.preferredLanguage)} />
                  )} />
                </div>
